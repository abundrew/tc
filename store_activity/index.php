...
require_once('store_activity.inc.php');

$now = NTPTime::get_time();

if (isset($_POST['excel']))
{
    BOActivity::activity_store_export_to_excel($now, $now, $check->get_store_id());
}

if (isset($_POST['pdf']))
{
    BOActivity::activity_store_export_to_fpdf($now, $now, $check->get_store_id());
}
...
page_head('Activity Report...', $check->is_admin(), $check->is_store(), $web_path.'help/store_activity.html');
...
echo BOActivity::activity_store_html_table($now, $now, $check->get_store_id());
echo '</table><br>';
